...
sys.path.append('/content/EMAGE-hf')
os.chdir('/content/EMAGE-hf')
...
class BaseTrainer(object):
    ...
    def test_demo(self, epoch):
        ...
        return [render_vid_path, results_save_path+"res_"+test_seq_list.iloc[its]['id']+'.npz', results_save_path+"gt_"+test_seq_list.iloc[its]['id']+'.npz']
...
class Predictor(BasePredictor):
    ...
    def predict(
        self,
        audio_path: Path = Input(description="Input Audio"),
    ) -> List[Path]:
        results = emage(str(audio_path))
        return [Path(results[0]), Path(results[1]), Path(results[2])]